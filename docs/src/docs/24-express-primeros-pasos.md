...
Está construido con **Bun** como runtime y gestor de paquetes, **Express 5.2.1** como framework HTTP, **Prisma 6** como ORM sobre **PostgreSQL**, **Zod** para validación, **jsonwebtoken** para tokens, **bcrypt** para hash de contraseñas y **pino** para logs.
...
- **presentation/** — DTOs de validación (Zod), controladores HTTP y definición de rutas Express.
...
3. **Sin librería de sesiones ni Redis**: la sesión de refresco es una fila en PostgreSQL. Simple, auditab<le y suficiente para este alcance.
...
Toda la configuración inicial vive en la raíz de example-project-express/: package.json define las dependencias y scripts, tsconfig.json configura TypeScript estricto para ejecución nativa del runtime (con imports relativos y extensión .ts explícita), tsup.config.ts arma el build de producción, .env.example documenta las variables de entorno y .gitignore mantiene fuera del repo lo que no debe versionarse.

> **Cómo crear el proyecto desde cero:** escribí estos archivos a mano en una carpeta vacía example-project-express/ y luego ejecuté los comandos de instalación. No es necesario usar un scaffolding (create-express-app, etc.): el proyecto es chico y entender cada archivo de configuración es parte del aprendizaje.

### 02.1 `package.json` — scripts, dependencias y tipado

> **package.json — scripts, dependencias y tipado**  
> Ruta: `example-project-express/package.json`  
> 
```json
{
  "name": "example-project-express",
  "version": "1.0.0",
  "type": "module",
  "scripts": {
...
- **dependencias de runtime**: express (framework web), los middlewares oficiales (helmet, cors, compression, cookie-parser, express-rate-limit), @prisma/client (ORM), zod (validación), jsonwebtoken (JWT), bcrypt (hash de contraseñas), pino y pino-http (logs) y dotenv (variables de entorno).
- **dependencias de desarrollo**: typescript, tsup (bundler), pino-pretty (logs bonitos en consola), prisma (CLI de migraciones) y los tipos (@types/bcrypt, @types/jsonwebtoken, @types/node, @types/express, @types/cors, @types/compression, @types/cookie-parser, bun-types).

> Nota: instalé con bun install, lo que genera bun.lock en lugar de package-lock.json.

### 02.2 `tsconfig.json` — TypeScript estricto para ejecución nativa

> **tsconfig.json — TypeScript estricto para ejecución nativa**  
> Ruta: `example-project-express/tsconfig.json`  
> 
```json
{
  "compilerOptions": {
...
Este tsconfig sigue la guía oficial de TypeScript para Node.js con type stripping: el runtime ejecuta .ts directamente, así que tsc solo chequea tipos (noEmit). Los flags más importantes:

- **"strict": true** — modo estricto completo; el compilador es exigente y eso evita errores antes de ejecutar.
- **"module": "nodenext"** con **"rewriteRelativeImportExtensions": true** — los imports internos llevan extensión .ts explícita (ej: import { env } from "../config/env.ts") y el runtime los resuelve tal cual. Con este esquema NO se usa alias @/: los imports son relativos y la extensión .ts es obligatoria porque TypeScript no reescribe paths de alias en modo type-stripping.
- **"erasableSyntaxOnly": true** — prohíbe sintaxis que no se puede borrar limpiamente (enums, namespaces, parámetros con propiedad): el código debe ser 100% type-stripping-compatible.
- **"verbatimModuleSyntax": true** — los imports de solo tipos necesitan la palabra clave type (import type), para que el runtime pueda borrarlos sin ambigüedad.
- **"types": ["bun-types"]** — trae los tipos globales de Bun (bun:test, etc.).
- **"noEmit": true** — tsc solo chequea tipos; quien corre el código es Bun en dev y tsup en build.

> La misma base de código corre en Bun (bun dev) y en Node.js 22.18+ (que también hace type stripping). Por eso los imports con .ts son la convención: funcionan igual en ambos runtime sin paso de compilación.

### 02.3 `tsup.config.ts` — bundler de producción

> **tsup.config.ts — bundler de producción**  
> Ruta: `example-project-express/tsup.config.ts`  
> 
```typescript
...
tsup empaqueta el servidor en un solo archivo ESM (dist/server.js) listo para correr en producción: entry es el punto de entrada, format esm, target es2022, sourcemap para debuggear y clean borra dist/ antes de cada build. No hace falta declarar el alias @/ acá porque los imports ya son relativos con extensión .ts (el mismo estilo que usa el runtime).
...
> Ruta: `example-project-express/.env.example`  
> 
```bash
# Entorno
...
# Base de datos
DATABASE_URL=postgresql://usuario:password@localhost:5432/example_project_express?schema=public
...
- **DATABASE_URL** — cadena de conexión a PostgreSQL (la base se llama example_project_express).
...
> Ruta: `example-project-express/.gitignore`  
> 
```bash
node_modules/
dist/
...
> Ruta: `example-project-express/prisma/schema.prisma`  
> 
```json
generator client {
...
> Ruta: `example-project-express/prisma/migrations/20260910013256_init/migration.sql`  
> 
```sql
-- CreateTable
CREATE TABLE "role" (
...
> Ruta: `example-project-express/src/config/env.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/config/logger.ts`  
> 
```typescript
...
pino es el logger más rápido de Node y pino-http lo adapta como middleware HTTP de Express. Dos exportaciones:
...
- **logger** — la instancia pino(loggerOptions), que se inyecta en pinoHttp (middleware que loggea cada request HTTP automáticamente) y se usa directo en procesos que no pasan por HTTP (graceful shutdown, errorHandler).

> pino-http loggea método, URL, status y duración de cada request apenas el middleware está montado en app.use. El logger de pino se comparte: mismo formato en requests HTTP y en logs de aplicación.

### 04.3 `src/config/prisma.ts` — cliente de Prisma compartido

> **prisma.ts — cliente de Prisma compartido**  
> Ruta: `example-project-express/src/config/prisma.ts`  
> 
```typescript
...
> El estilo de imports de todo el proyecto se ve acá: src/config/prisma.ts importa con rutas relativas y extensión .ts explícita (import { prisma } from "./prisma.ts"), la misma convención de los módulos de negocio.

### 04.4 `src/config/graceful-shutdown.ts` — apagado elegante

> **graceful-shutdown.ts — apagado elegante**  
> Ruta: `example-project-express/src/config/graceful-shutdown.ts`  
> 
```typescript
...
La estrategia de errores es la columna del proyecto: todo error de negocio es una instancia de AppError (o subclase) con statusCode, code y message; el errorHandler de Express los serializa a JSON. Los errores inesperados (bugs) se loggean y devuelven 500 INTERNAL_ERROR sin filtrar detalles internos.
...
> Ruta: `example-project-express/src/core/errors/AppError.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/core/errors/error-messages.ts`  
> 
```typescript
...
### 05.3 `src/config/error-handler.ts` — el errorHandler de Express

> **error-handler.ts — el errorHandler de Express**  
> Ruta: `example-project-express/src/config/error-handler.ts`  
> 
```typescript
...
Express identifica un middleware de error por su FIRMA de 4 parámetros (error, req, res, next) — obligatorio que sean 4 aunque no se usen todos, porque Express diferencia los middlewares normales (3 params) de los de error (4 params). Se registra con app.use(errorHandler) y es el ÚLTIMO middleware de la cadena. Este archivo hace tres chequeos en orden:

1. **SyntaxError con status 400** → 400 BAD_REQUEST "Invalid JSON body" (express.json() lanza este error cuando el body no es JSON válido; Express 5 lo reenvía al error handler automáticamente).
2. **ZodError** → 400 BAD_REQUEST con el primer issue formateado como ruta:mensaje (ej: "product_id: Invalid product id"). Así el cliente sabe exactamente qué campo falló.
3. **AppError** → statusCode + { code, message, details? }.
4. **Cualquier otra cosa** → se loggea con pino (con el error completo y la request que lo causó: método, URL) y se responde 500 INTERNAL_ERROR genérico. Nunca se filtra el stack al cliente.

> En Express 5 los handlers async que lanzan errores se propagan solos al middleware de error (no hace falta next(err) manual ni wrapper). El orden importa: ZodError se chequea antes de AppError porque Zod no extiende AppError.

## 06. Punto de entrada: de arranque a rutas

La app arranca en src/server.ts, se construye en src/app.ts (middlewares globales + error handler) y monta los routers en src/http/routes.ts. La separación buildApp() / server.ts permite testear la app sin abrir un puerto real (buildApp devuelve la app de Express lista para montar o escuchar).

### 06.1 `src/server.ts` — bootstrap del servidor

> **server.ts — bootstrap del servidor**  
> Ruta: `example-project-express/src/server.ts`  
> 
```typescript
...
1. buildApp() construye la app de Express completa (middlewares + rutas + error handler).
2. app.listen({ port, host }) abre el puerto y el callback loggea la URL. app.listen devuelve el Server HTTP de Node (node:http), que es lo que necesita gracefulShutdown para cerrar conexiones.
3. gracefulShutdown(server) registra los handlers de señales.

> Top-level await: server.ts es un módulo ESM, así que la inicialización corre directo en el módulo sin wrapper async.

### 06.2 `src/app.ts` — fábrica de la aplicación Express

> **app.ts — fábrica de la aplicación Express**  
> Ruta: `example-project-express/src/app.ts`  
> 
```typescript
...
buildApp() arma la app y encadena los middlewares GLOBALES con app.use — el orden importa porque Express los ejecuta en secuencia:

- **app.disable("x-powered-by")** — saca el header X-Powered-By: Express (no regalar info del stack).
...
- **express.json()** — parsea el body JSON y lo deja en req.body; si el JSON es inválido lanza el SyntaxError que el errorHandler convierte en 400 "Invalid JSON body".
- **cookieParser()** — parsea request.cookies y agrega res.cookie/clearCookie (necesario para los tokens httpOnly).
- **pinoHttp({ logger })** — loggea cada request automáticamente.
- **rateLimit({...})** — 100 requests por minuto por IP (protección básica contra fuerza bruta en /auth/login).

Después montamos las rutas (registerRoutes) y el errorHandler como ÚLTIMO middleware. Si querés sumar un middleware nuevo (ej: express-session), este es el lugar.

### 06.3 `src/http/routes.ts` — registro de routers con prefijo

> **routes.ts — registro de routers con prefijo**  
> Ruta: `example-project-express/src/http/routes.ts`  
> 
```typescript
...
Cada módulo exporta su router de Express (authRoutes, rolesRoutes, ...) y acá se monta con app.use("<prefijo>", router): /auth, /roles, /users, /products, /services, /inventory. Un módulo nuevo se agrega con una línea: import + app.use.
...
> Ruta: `example-project-express/src/types/auth.d.ts`  
> 
```typescript
...
### 07.2 `src/types/express.d.ts` — extension de ExpressRequest

> **express.d.ts — extension de ExpressRequest**  
> Ruta: `example-project-express/src/types/express.d.ts`  
> 
```typescript
declare global {
  namespace Express {
...
> Ruta: `example-project-express/src/modules/auth/domain/auth.entities.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/auth/domain/auth.interface.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/auth/application/common/crypto.utils.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/auth/application/common/token.utils.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/auth/application/common/cookie.utils.ts`  
> 
```typescript
...
setAuthCookies usa res.cookie con maxAge en MILISEGUNDOS (convención de Express): access_token expira en 15 * 60 * 1000 (15 minutos) y refresh_token en 7 * 24 * 60 * 60 * 1000 (7 días) — las constantes están arriba del archivo. Misma base de opciones para ambas:
...
> Ruta: `example-project-express/src/modules/auth/application/common/auth.guard.ts`  
> 
```typescript
import type { NextFunction, Request, Response } from "express"
...
authGuard y adminGuard son middlewares de Express (reciben req, res, next) y se encadenan en las rutas que protegen:
...
- **adminGuard** — corre DESPUÉS de authGuard (los middlewares corren en orden) y tira ForbiddenError si el role no es "admin".

> Uso en rutas: router.get("/", authGuard, adminGuard, handler) protege y restringe; router.get("/", authGuard, handler) solo autentica. En Express el middleware se pasa como argumento de la ruta (o en array: [authGuard, adminGuard]).
...
> Ruta: `example-project-express/src/modules/auth/application/auth.service.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/auth/infrastructure/auth.prisma.repository.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/auth/presentation/auth.dto.ts`  
> 
```typescript
import { z } from "zod"
...
> Ruta: `example-project-express/src/modules/auth/presentation/auth.controller.ts`  
> 
```typescript
...
- login → setAuthCookies(res, ...) y devuelve solo el perfil (los tokens no se exponen en el body porque ya van en cookies; el client curl puede igualmente leerlos del cookie jar). El handler es async y Express 5 propaga el rechazo automáticamente al errorHandler (no hace falta try/catch).
...
> Ruta: `example-project-express/src/modules/auth/presentation/auth.routes.ts`  
> 
```typescript
...
4. **presentation/** — DTOs Zod + controller + rutas Express.
...
> Ruta: `example-project-express/src/modules/roles/domain/roles.entities.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/roles/domain/roles.types.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/roles/domain/roles.interface.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/roles/application/common/roles.mappers.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/roles/application/roles.service.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/roles/infrastructure/roles.prisma.repository.ts`  
> 
```typescript
import { Prisma } from "@prisma/client"
import { prisma } from "../../../config/prisma.ts"
...
> Ruta: `example-project-express/src/modules/roles/presentation/roles.dto.ts`  
> 
```typescript
import { z } from "zod"
...
> Ruta: `example-project-express/src/modules/roles/presentation/roles.controller.ts`  
> 
```typescript
import type { Request, Response } from "express"
...
- getById → request.params as { id: string } (Express no tipea params por defecto) → service.getById, 200.
...
> Ruta: `example-project-express/src/modules/roles/presentation/roles.routes.ts`  
> 
```typescript
import { Router } from "express"
...
GET / (listar), GET /:id, POST / (crear), PUT /:id (actualizar), DELETE /:id (borrar). Todas pasan por los middlewares authGuard y adminGuard (router.get("/", authGuard, adminGuard, handler)) — primero autentica, después verifica que sea admin. Así de simple queda la autorización por rol.
...
> Ruta: `example-project-express/src/modules/users/domain/users.entities.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/users/domain/users.types.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/users/application/common/users.mappers.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/users/application/users.service.ts`  
> 
```typescript
import { ConflictError, NotFoundError } from "../../../core/errors/AppError.ts"
...
> Ruta: `example-project-express/src/modules/users/infrastructure/users.prisma.repository.ts`  
> 
```typescript
import { Prisma } from "@prisma/client"
import { prisma } from "../../../config/prisma.ts"
...
> Ruta: `example-project-express/src/modules/users/presentation/users.dto.ts`  
> 
```typescript
import { z } from "zod"
...
> Ruta: `example-project-express/src/modules/users/presentation/users.controller.ts`  
> 
```typescript
import type { Request, Response } from "express"
...
> Ruta: `example-project-express/src/modules/users/presentation/users.routes.ts`  
> 
```typescript
import { Router } from "express"
...
> Ruta: `example-project-express/src/modules/products/domain/products.entities.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/products/domain/products.types.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/products/application/common/products.mappers.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/products/application/products.service.ts`  
> 
```typescript
import { Prisma } from "@prisma/client"
import { ConflictError, NotFoundError } from "../../../core/errors/AppError.ts"
...
> Ruta: `example-project-express/src/modules/products/infrastructure/products.prisma.repository.ts`  
> 
```typescript
import { Prisma } from "@prisma/client"
import { prisma } from "../../../config/prisma.ts"
...
> Ruta: `example-project-express/src/modules/products/presentation/products.dto.ts`  
> 
```typescript
import { z } from "zod"
...
> Ruta: `example-project-express/src/modules/products/presentation/products.controller.ts`  
> 
```typescript
import type { Request, Response } from "express"
...
> Ruta: `example-project-express/src/modules/products/presentation/products.routes.ts`  
> 
```typescript
import { Router } from "express"
...
El CRUD completo con middleware authGuard — cualquier usuario logueado (admin o staff) puede listar, crear, editar y borrar productos. La decisión de quién puede qué se nota acá: inventario/catálogo para staff, roles/usuarios solo admin.
...
> Ruta: `example-project-express/src/modules/services/domain/services.entities.ts`  
> 
```typescript
import type { Decimal } from "@prisma/client/runtime/library"
...
> Ruta: `example-project-express/src/modules/services/application/common/services.mappers.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/services/application/services.service.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/services/infrastructure/services.prisma.repository.ts`  
> 
```typescript
import { Prisma } from "@prisma/client"
import { prisma } from "../../../config/prisma.ts"
...
> Ruta: `example-project-express/src/modules/services/presentation/services.dto.ts`  
> 
```typescript
import { z } from "zod"
...
> Ruta: `example-project-express/src/modules/services/presentation/services.controller.ts`  
> 
```typescript
import type { Request, Response } from "express"
...
> Ruta: `example-project-express/src/modules/services/presentation/services.routes.ts`  
> 
```typescript
import { Router } from "express"
...
> Ruta: `example-project-express/src/modules/inventory/domain/inventory.entities.ts`  
> 
```typescript
import type { Decimal } from "@prisma/client/runtime/library"
...
> Ruta: `example-project-express/src/modules/inventory/domain/inventory.types.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/inventory/application/common/inventory.mappers.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/inventory/application/inventory.service.ts`  
> 
```typescript
import { ConflictError, NotFoundError } from "../../../core/errors/AppError.ts"
...
> Ruta: `example-project-express/src/modules/inventory/infrastructure/inventory.prisma.repository.ts`  
> 
```typescript
import { Prisma } from "@prisma/client"
import { prisma } from "../../../config/prisma.ts"
...
> Ruta: `example-project-express/src/modules/inventory/presentation/inventory.dto.ts`  
> 
```typescript
import { z } from "zod"
...
> Ruta: `example-project-express/src/modules/inventory/presentation/inventory.controller.ts`  
> 
```typescript
import type { Request, Response } from "express"
...
> Ruta: `example-project-express/src/modules/inventory/presentation/inventory.routes.ts`  
> 
```typescript
import { Router } from "express"
...
> Ruta: `example-project-express/src/scripts/seed.ts`  
> 
```typescript
import { PrismaClient } from "@prisma/client"
import bcrypt from "bcrypt"
...
> Ruta: `example-project-express/src/tests/fakes.ts`  
> 
```typescript
import { Prisma } from "@prisma/client"
...
> Ruta: `example-project-express/src/modules/auth/_tests_/application/auth.service.test.ts`  
> 
```typescript
...
> Ruta: `example-project-express/src/modules/roles/_tests_/application/roles.service.test.ts`  
> 
```typescript
import { test } from "bun:test"
import assert from "node:assert/strict"
...
> Ruta: `example-project-express/src/modules/users/_tests_/application/users.service.test.ts`  
> 
```typescript
import { test } from "bun:test"
import assert from "node:assert/strict"
...
> Ruta: `example-project-express/src/modules/products/_tests_/application/products.service.test.ts`  
> 
```typescript
import { test } from "bun:test"
import assert from "node:assert/strict"
...
> Ruta: `example-project-express/src/modules/services/_tests_/application/services.service.test.ts`  
> 
```typescript
import { test } from "bun:test"
import assert from "node:assert/strict"
...
> Ruta: `example-project-express/src/modules/inventory/_tests_/application/inventory.service.test.ts`  
> 
```typescript
import { test } from "bun:test"
import assert from "node:assert/strict"
...
> Ruta: `example-project-express/http/auth.http`  
> 
```http
### 1. Login as admin
...
> Ruta: `example-project-express/http/roles.http`  
> 
```http
...
> Ruta: `example-project-express/http/users.http`  
> 
```http
...
> Ruta: `example-project-express/http/products.http`  
> 
```http
...
> Ruta: `example-project-express/http/services.http`  
> 
```http
### 1. List services
...
> Ruta: `example-project-express/http/inventory.http`  
> 
```http
